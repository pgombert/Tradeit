...
import { getLatestMorningReview } from '../services/morning-review.service.js';
...
// The weekly brief. `/brief` serves the latest *stored* brief — the full chain
// including the AI analyst and red team — generated by the `brief` job. Until
...
// The latest stored morning check — every holding's Hold/Trim/Exit call against
// the day's data, generated by the `morning` job. Advisory; never trades.
router.get('/morning-review', asyncHandler(async (_req, res) => {
  const review = await getLatestMorningReview();
